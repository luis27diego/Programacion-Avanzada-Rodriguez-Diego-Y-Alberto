...
def crear_figura_torta(datos_torta: Dict[str, float]):
    """Crea y devuelve una figura de Plotly con gráfico de torta."""
    if not datos_torta:
        return None
...
def crear_figura_barra_mediana(mediana_en_proceso: float, mediana_resueltos: float, mediana_pendiente: float):
    """Crea y devuelve una figura de Plotly con gráfico de barras."""
    fig = px.bar(
...
def crear_barra_mediana(mediana_en_proceso: float, mediana_resueltos: float, mediana_pendiente: float) -> str:
    ...
    fig = crear_figura_barra_mediana(mediana_en_proceso, mediana_resueltos, mediana_pendiente)
    return pio.to_html(fig, full_html=False, config={'responsive': True})
...
def crear_imagen_nube_palabras(datos_palabras: List[Tuple[str, int]]) -> str:
    ...
    fig.savefig(ruta_imagen, bbox_inches='tight', dpi=100, facecolor='white')
    plt.close(fig)
