...
# 5. Agregar adhesión (jefe adhiere a reclamo1)
reclamo_repo.crear_relacion(2, 1, 'adhesion')  # Jefe (id=2) adhiere a reclamo1 (id=1)
reclamo = reclamo_repo.obtener_registro_por_filtro('id', 1)
if reclamo:
    user_jefe = usuario_repo.obtener_registro_por_filtro('id', 2)
    if user_jefe:
        print(f"Reclamo1 tiene {reclamo.cantidad_adherentes()} adherente(s).")

# 6. Cambiar estado (solo jefe o secretario)
jefe = usuario_repo.obtener_registro_por_filtro('id', 2)
if jefe and reclamo:
